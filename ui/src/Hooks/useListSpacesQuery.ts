...
export type ListSpacesQuery = GenericQuery<Space[]>;
...
const useListSpacesQuery = (): ListSpacesQuery => {
  ...
  const fetch = useMemo(() => async () => {
    try {
      ...
      return sortSpaces(data.data);
    } catch (e) {
      const err = e as APIError;
      if (err.response && err.response.status === 403) {
        return []; // authorization is handled by unauthorizedRequestResponseHandler
      } else {
        throw e;
      }
    }
  ...
  return useGenericQuery<Space[]>(fetch);
};
